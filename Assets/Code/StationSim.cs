...
public class StationSim : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //Morale
        stats[3].text = morale + "%";
        stats[3].color = new Color(1 - Mathf.Lerp(0, 1, ((float)morale / (float)100f)), Mathf.Lerp(0, 1, ((float)morale / (float)100f)), 0);

        //COS
        stats[4].text = COS + "%";
        stats[4].color = new Color(1 - Mathf.Lerp(0, 1, ((float)COS / (float)100f)), Mathf.Lerp(0, 1, ((float)COS / (float)100f)), 0);
    }
}
